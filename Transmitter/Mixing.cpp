...
{
    int8_t y = (k == 0) ? x : (k * x * x * x / (HIGH_END_LIMIT * HIGH_END_LIMIT) + x * (100 - k)) / 100;
...
void applyModelSettings()
{
    ...
    //Throttle channel
    int val = map(txData.channel[0], LOW_END_LIMIT, HIGH_END_LIMIT, 0, 2*modelConfig.highEndpoint[0]);
